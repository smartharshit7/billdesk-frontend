      ...
      <div style={{...card(),borderRadius:16,padding:"40px 36px",width:"100%",maxWidth:420}} className="fade-in">
        ...
        <div style={{display:"flex",background:"#0f1117",borderRadius:10,padding:4,marginBottom:24,border:"1px solid #2a2d3a"}}>
          {[["login","Sign In"],["register","Register"]].map(([v,l])=>(
            <button key={v} onClick={()=>{setMode(v);setErr("");}} style={{flex:1,padding:"8px 0",borderRadius:7,border:"none",background:mode===v?"#7c6af7":"transparent",color:mode===v?"#fff":"#7a7d8f",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:500,transition:"all .2s"}}>{l}</button>
          ))}
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div>
            <label style={lbl}>Username *</label>
            <input style={inp} value={f.username} onChange={e=>setFld("username",e.target.value)} onKeyDown={e=>e.key==="Enter"&&submitCreds()} placeholder="your_username" autoFocus/>
          </div>
          {mode==="register" && (
            <>
          ...
          <div style={{textAlign:"center",color:"#7a7d8f",fontSize:12}}>— or —</div>
          <GoogleSignInBtn onSuccess={u=>{onLogin(u);}} onError={e=>setGErr(e)}/>
          {gErr&&<p style={{color:"#e05252",fontSize:12,textAlign:"center"}}>{gErr}</p>}
        </div>
      </div>
    </div>
  );
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            setPayMsg({
              type: "success",
              text: result.trialStillActive
                ? `🎉 Subscribed! Your ${plan} plan activates after your trial ends on ${new Date(result.paidStart).toLocaleDateString("en-IN",{day:"numeric",month:"long",year:"numeric"})}.`
                : `🎉 Subscribed! Your ${plan} plan is now active until ${new Date(result.paidEnd).toLocaleDateString("en-IN",{day:"numeric",month:"long",year:"numeric"})}.`,
            });
            await fetchStatus();
          } catch(e) {
        ...
        <div style={{background:"#181a23",border:`1px solid ${statusColor(subStatus.status)}40`,borderRadius:14,padding:"20px 24px",marginBottom:36,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:16}}>
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            ...
            <div>
              ...
              {subStatus.status==="trialing" && subStatus.paidEnd && (
                <div style={{color:"#43c59e",fontSize:12,marginTop:4}}>
                  ✓ Paid plan queued — activates {fmtDate(subStatus.paidEnd ? subStatus.trialEnd : null)} after trial
                </div>
              )}
            </div>
          </div>
            ...
            <div key={plan.id} className={`plan-card${plan.popular?" popular-glow":""}`}
              ...
              {isCurrentPlan ? (
                ...
                <button
                  ...
                  disabled={!!paying}
                  style={{...btn(plan.popular?"#7c6af7":"#2a2d3a",plan.popular?"#fff":"#e8e9f0",{width:"100%",padding:"13px",fontSize:14,fontWeight:600,opacity:paying?0.6:1,border:plan.popular?"none":"1px solid #3a3d4a"})}}
                >
